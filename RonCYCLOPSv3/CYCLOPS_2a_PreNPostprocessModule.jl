...
function GetEigenGenes(numeric_data::Array{Float64,2},fraction_var=.75, dfrac_var=0.05,maxeig=50)
    ...
    eigen_sig_fraction = (w.^2)/sum(w.^2)                                                         ##modification here
    ...
    ReductionDim, 10*Transform, eigen_sig_fraction, eigen_sum_fraction                            ##modification here
end
...
c2=@spawn MultiCore_Cosinor_Statistics(alldata_data[((1+1*rowbin):(2*rowbin)),:],estimated_phaselist); 
c3=@spawn MultiCore_Cosinor_Statistics(alldata_data[((1+2*rowbin):(3*rowbin)),:],estimated_phaselist);
c4=@spawn MultiCore_Cosinor_Statistics(alldata_data[((1+3*rowbin):(4*rowbin)),:],estimated_phaselist); 
c5=@spawn MultiCore_Cosinor_Statistics(alldata_data[((1+4*rowbin):ngenes),:],estimated_phaselist);
